...
/// <summary>
/// Tests for the auto-generated record members of <see cref="PreLoadActionParameters"/>.
/// We construct with a <c>null!</c> SqlConnection because tests don't actually invoke
/// any SQL — they just exercise the record's compiler-generated equality, hashing,
/// ToString, and deconstruct members.
/// </summary>
public class PreLoadActionParametersTests
{
    private static PreLoadActionParameters CreateSut()
    {
        var idProp = typeof(TestRecord).GetProperty(nameof(TestRecord.Id))!;
        var columns = new List<ColumnMap> { new(idProp, ordinal: 0) };

        return new PreLoadActionParameters
        (
            Connection: null!,
            Transaction: null,
            SchemaName: "dbo",
    ...
    public void ToString_returns_non_empty_string_with_record_name()
    {
        ...
        Assert.Contains(nameof(PreLoadActionParameters), text, StringComparison.Ordinal);
    }
